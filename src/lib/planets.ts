...
type FullClass = { class: string; description: string }
...
export function generatePlanet(): Planet {
    const planetClass: FullClass = randomClass();

    switch (planetClass.class) {
        case 'Rocky':
            // Generate planet classes based on how likely they are with a with planet of this class
            // Note: also enforce mutual exclusion

            // Generate planet id with a random 5 character string or a name

            // Generate planet mass, density, diameter, and rotational speed.
